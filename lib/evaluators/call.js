...
module.exports = BaseEvaluator.extend({
	...
	"run": function(expression, context, next) {
		...
		//make the args into a context so it's easier to handle. Note that
		//the we merge the context into the given target since there are many
		//cases where data from the context might be used in the target - but we
		//also want to make sure they're kept separate incase the context is something
		//we want to modify. E.g: run("log", "hello world!")
