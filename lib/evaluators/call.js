...
module.exports = BaseEvaluator.extend({
	...
	"run": function(expression, target, next) {
		...
		callTarget.data = tpl.render(callTarget.data, target.data);
		callTarget.value = tpl.render(callTarget.value, target.data);
		callTarget.defaults(target);


		this.evaluator.commands.run(this._relativeCommand(expression, command), callTarget, outcome.error(next).success(function() {
			next(null, false);
		}));
	},
